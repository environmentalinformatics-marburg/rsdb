...
extent_radius <- function(x, y, r) {
  return(raster::extent(x-r,x+r,y-r,y+r))
}
...
as.speclib <- function(rasterStack, na=NULL) {
  ...
  meta <- hsdar::rastermeta(rasterStack, crs=raster::crs(rasterStack))
  sp <- hsdar::speclib(spectra=m, wavelength=wavelengths, fwhm=fwhms, rastermeta=meta)
  return(sp)
}
