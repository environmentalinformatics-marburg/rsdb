<template>
    <span  style="display: inline-block;">
        <v-dialog v-model="dialog" lazy>
            <v-btn title="Show URL for WMS based access to this VectorDB layer." slot="activator">
                <v-icon left>folder_open</v-icon>WMS access
            </v-btn>
            <v-card>
                <v-card-title>
                    <div class="headline">WMS access of <i>VectorDB</i>&nbsp;&nbsp;&nbsp;<b>{{meta.name}}</b></div>
                </v-card-title>
                WMS URL:
                <v-btn @click="onUrlCopy" title="copy WMS URL of this VectorDB layer to clipboard"><v-icon>content_copy</v-icon> Copy to clipboard</v-btn>
                <div class="wms-url">{{wmsUrl}}</div>

                <br>
                Web Feature Service (WMS) <a href="https://www.ogc.org/standards/wms" target="_blank">specification</a>, 
                ...
                At Qgis window <i>Browser</i>-Box rigth click on <i>WMS</i>-list-entry,
                click the shown <i>New Connection...</i>-Button,
                the <i>Create a New WMS Connection</i>-dialog opens,
                ...
                <br>
                <br>The new layer is shown as list-entry in the sub-list of the <i>WMS</i>-list-entry,
                ...
                <br>Cache is now cleared. Right click on the WMS-entry at the Browser-box, click <i>refresh</i>-button to load updated data.
...
export default {
    name: 'admin-vectordb-dialog-wms',
    props: ['meta'],        
    components: {
    },
    data() {
    ...
    computed: {
        ...
        wmsUrl() {
            return this.identity === undefined || this.meta === undefined ? '[unknown]' : (this.identity.url_base + '/vectordbs/' + this.meta.name + '/wms');
        },
    },
    watch: {
        meta() {
