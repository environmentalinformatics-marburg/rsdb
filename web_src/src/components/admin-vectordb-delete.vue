<template>
    <span>
        <v-dialog v-model="dialog" lazy absolute width="800px">
            <v-btn title="Delete this vector layer. This can not be undone." slot="activator">
                <v-icon left>delete_forever</v-icon>delete entire layer
            </v-btn>
            <v-card>
                <v-card-title>
                    <div class="headline">DELETE <i>VectorDB</i>&nbsp;&nbsp;&nbsp;<b>{{meta.name}}</b></div>
                </v-card-title>
                <v-card-text>
                    Delete this vector layer. <b>This can not be undone.</b>
                </v-card-text>
                <v-card-text>
                    Are you sure?
                </v-card-text>
