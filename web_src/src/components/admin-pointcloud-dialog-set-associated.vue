<template>
    <span>
        <v-dialog v-model="dialog" lazy absolute width="800px">
            ...
            <v-card>
                <v-card-title>
                    <div class="headline">Set Associated layers of <i>PointCloud</i>&nbsp;&nbsp;&nbsp;<b>{{meta.name}}</b></div>
                </v-card-title>
                <v-card-text style="min-height: 400px;">  
                
                    RasterDB-layer: Raster-visualisation of this pointcloud.
                   <multiselect v-model="selectedRasterdb" :options="rasterdbs" placeholder="select rasterdb-layer" :allow-empty="true"/>
                   <br> 
                   POI-groups: Groups of named geo-points that can be used for processing this pointcloud.
                   <multiselect v-model="selectedPoi_groups" :options="poi_groups" placeholder="select poi-groups" :allow-empty="true" :multiple="true"/>
                   <br> 
                   ROI-groups: Groups of named polygons that can be used for processing this pointcloud.
...
export default {
    name: 'admin-pointcloud-dialog-set-associated',
    props: ['meta'],
