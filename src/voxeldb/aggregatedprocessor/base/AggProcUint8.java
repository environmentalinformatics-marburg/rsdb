...
public abstract class AggProcUint8 extends AggProc {

	protected byte[][][] dst;

	public AggProcUint8(CellFactory cellFactory, Range3d range, int aggregation_factor_x, int aggregation_factor_y, int aggregation_factor_z, VoxelGeoRef aggRef) {
		super(cellFactory, range, aggregation_factor_x, aggregation_factor_y, aggregation_factor_z, aggRef);
		this.cellFactory.setCount();
		dst = new byte[zAggLen][yAggLen][xAggLen];	
	}
	...
	public void write(Response response, String format, boolean crop) throws IOException {
		...
		VoxelWriter.writeUint8(data, cellFactory.getvoxeldb().getName(), aggRef, aggRange, response, format);		
	}
}
