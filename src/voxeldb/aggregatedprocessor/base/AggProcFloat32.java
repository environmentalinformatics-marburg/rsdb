...
public abstract class AggProcFloat32 extends AggProc {

	protected float[][][] dst;

	public AggProcFloat32(CellFactory cellFactory, Range3d range, int aggregation_factor_x, int aggregation_factor_y, int aggregation_factor_z, VoxelGeoRef aggRef) {
		super(cellFactory, range, aggregation_factor_x, aggregation_factor_y, aggregation_factor_z, aggRef);
		this.cellFactory.setCount();
		dst = new float[zAggLen][yAggLen][xAggLen];	
	}
	...
	public void write(Response response, String format, boolean crop) throws IOException {
		...
		VoxelWriter.writeFloat32(data, cellFactory.getvoxeldb().getName(), aggRef, aggRange, response, format);		
	}
}
