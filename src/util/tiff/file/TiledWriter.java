...
public class TiledWriter {
	...
	public static void main(String[] args) throws IOException {
		...
		try(Broker broker = new Broker()) {
			...
			try(RandomAccessFile raf = new RandomAccessFile("temp/testingTiff.tif", "rw")) {
				raf.setLength(0);
				long imageDataPos = tiffFile.writeHeader(raf);
				raf.seek(imageDataPos);

				ReadonlyArrayIterator<TiffTile[][]> it = tiffFile.overviewtilesVec.iterator();
				TiffTile[][] tiles = tiffFile.tiles;
				while(true) {
					log.info("process scale " + scale);
