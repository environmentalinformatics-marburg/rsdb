...
class PointImageIntensity extends PointImageAbstract {

	public PointImageIntensity(long utmm_min_x, long utmm_min_y, int screen_width, int screen_height, int TILE_LOCAL_TO_SCREEN_DIV, ImageType imageType, Statistics tileMinMaxCalc) {
	...
	public void nextPoint(Point point) {
		...
		int c = point.intensity;
		if(c!=0) {
			//imageBuffer[y*width+x] = MonoColor.getSafeRGBA(MonoColor.pow((c-intensity_min) * z_rangeInv, ginv_itensity));
			imageBuffer[y*width+x] = MonoColor.getSafeRGBA256((int) ((c-intensity_min) * intensity_rangeInv255));
		}
	}
}