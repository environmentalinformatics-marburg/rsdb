...
public class Rasterizer {	
	...
	public static ImageBufferARGB rasterizeRGB(TimeFrameProducer processor, TimeBand[] bands, int width, int height, double gamma, double[] range, boolean syncBands, Interruptor interruptor, int per_mille) {
		Timer.start("load");
		Logger.info(Arrays.toString(bands));
		if(bands[0].band.isPixelTypeInt16OrExactConvertible() && bands[1].band.isPixelTypeInt16OrExactConvertible() && bands[2].band.isPixelTypeInt16OrExactConvertible()) {
			if((bands[0].band.index == bands[1].band.index && bands[1].band.index != bands[2].band.index) || (bands[1].band.index == bands[2].band.index && bands[2].band.index != bands[0].band.index)) {
				...
				Interruptor.checkInterrupted(interruptor);
				return Renderer.renderRbShort(frameR, frameB, naR, naB, width, height, gamma, range, syncBands, per_mille);			
			} else {
				short naR = bands[0].band.getInt16NA();
				...
				//Logger.info(Timer.stop("load"));
				Interruptor.checkInterrupted(interruptor);
				return Renderer.renderRgbShort(frameR, frameG, frameB, naR, naG, naB, width, height, gamma, range, syncBands, per_mille);				
			}			
			...
			if((bands[0].band.index == bands[1].band.index && bands[1].band.index != bands[2].band.index) || (bands[1].band.index == bands[2].band.index && bands[2].band.index != bands[0].band.index)) {
				...
				//Logger.info(Timer.stop("load"));
				Interruptor.checkInterrupted(interruptor);
				return Renderer.renderRbFloat(frameR, frameB, width, height, gamma, range, syncBands, per_mille);		
			} else {
				Interruptor.checkInterrupted(interruptor);
				FloatFrame frameR = processor.getFloatFrame(bands[0]);
				Interruptor.checkInterrupted(interruptor);
				...
				Logger.info(Timer.stop("load"));
				Interruptor.checkInterrupted(interruptor);
				return Renderer.renderRgbFloat(frameR, frameG, frameB, width, height, gamma, range, syncBands, per_mille);
			}	
		} else {
			if((bands[0].band.index == bands[1].band.index && bands[1].band.index != bands[2].band.index) || (bands[1].band.index == bands[2].band.index && bands[2].band.index != bands[0].band.index)) {
				Interruptor.checkInterrupted(interruptor);
				...
				Logger.info(Timer.stop("load"));
				Interruptor.checkInterrupted(interruptor);
				return Renderer.renderRbDouble(frameR, frameB, width, height, gamma, range, syncBands, per_mille);		
			} else {
				Interruptor.checkInterrupted(interruptor);
				DoubleFrame frameR = processor.getDoubleFrame(bands[0]);
				Interruptor.checkInterrupted(interruptor);
				...
				Logger.info(Timer.stop("load"));
				Interruptor.checkInterrupted(interruptor);
				return Renderer.renderRgbDouble(frameR, frameG, frameB, width, height, gamma, range, syncBands, per_mille);
			}	
		}
	}

	public static ImageBufferARGB rasterizeGrey(TimeFrameProducer processor, TimeBand timeband, int width, int height, double gamma, double[] range, Interruptor interruptor, int per_mille) {
		if(timeband.band.isPixelTypeInt16OrExactConvertible())  {
			...
			Interruptor.checkInterrupted(interruptor);
			return Renderer.renderGreyShort(frame, na, width, height, gamma, range, per_mille);
		} else if(timeband.band.isPixelTypeFloat32OrExactConvertible())  {
			Interruptor.checkInterrupted(interruptor);
			FloatFrame frame = processor.getFloatFrame(timeband);
			Interruptor.checkInterrupted(interruptor);
			return Renderer.renderGreyFloat(frame, width, height, gamma, range, per_mille);		
		} else {
			Interruptor.checkInterrupted(interruptor);
			DoubleFrame frame = processor.getDoubleFrame(timeband);
			Interruptor.checkInterrupted(interruptor);
			return Renderer.renderGreyDouble(frame, width, height, gamma, range, per_mille);		
		}	
	}
	
	public static ImageBufferARGB rasterizePalette(TimeFrameProducer processor, TimeBand timeband, int width, int height, double gamma, double[] range, int[] palette, Interruptor interruptor, int per_mille) {
		if(timeband.band.isPixelTypeInt16OrExactConvertible())  {
			short na = timeband.band.getInt16NA();
			Interruptor.checkInterrupted(interruptor);
			ShortFrame frame = processor.getShortFrame(timeband);
			Interruptor.checkInterrupted(interruptor);
			return Renderer.renderPaletteShort(frame, na, width, height, gamma, range, palette, per_mille);
		} else if(timeband.band.isPixelTypeFloat32OrExactConvertible())  {
			Interruptor.checkInterrupted(interruptor);
			FloatFrame frame = processor.getFloatFrame(timeband);
			Interruptor.checkInterrupted(interruptor);
			return Renderer.renderPaletteFloat(frame, width, height, gamma, range, palette, per_mille);		
		} else {
			Interruptor.checkInterrupted(interruptor);
			DoubleFrame frame = processor.getDoubleFrame(timeband);
			Interruptor.checkInterrupted(interruptor);
			return Renderer.renderPaletteDouble(frame, width, height, gamma, range, palette, per_mille);		
		}	
	}
}
